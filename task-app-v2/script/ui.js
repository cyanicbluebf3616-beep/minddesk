function renderStats(elements) {
  ...
  const risk = targetTasks.filter(isRiskTask).length;
  const blocked = targetTasks.filter(task => task.status === "Blocked" || !!task.blockedReason).length;
  const pending = targetReports.filter(report => report.state === "pending").length;

  if (elements.statTotal) elements.statTotal.textContent = total;
  if (elements.statRisk) elements.statRisk.textContent = risk;
  if (elements.statBlocked) elements.statBlocked.textContent = blocked;
  if (elements.statPendingReports) elements.statPendingReports.textContent = pending;
}
        ...
        el.innerHTML = `
          ...
          <div class="priority-main">
            ...
            <div class="priority-meta">
              ...
              <span>期限: ${escapeHtml(task.dueDate || "未設定")}</span>
      ...
      elements.riskHero.innerHTML = `
        <div class="hero-alert">
          ...
          <div class="hero-sub">
            ...
            ${reasons} / 期限: ${top.dueDate || "未設定"} / 危険度: ${getRiskScore(top)}
          </div>
        </div>
      `;
    } else {
    ...
    card.innerHTML = `
      ...
      <div class="task-meta">
        ...
        <div class="meta">
          ...
          <div class="meta-value">${task.dueDate || "-"}</div>
        </div>
        <div class="meta">
