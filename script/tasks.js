...
function isDueSoon(task) {
  if (!task.finalDueDate || task.status === "Done") return false;
  const now = getNow();
...
function getFilteredTasks() {
  ...
  if (sortOrder === "dueSoon") {
    return list.sort((a, b) => {
      const aTime = a.finalDueDate ? new Date(a.finalDueDate + "T23:59:59").getTime() : Infinity;
      const bTime = b.finalDueDate ? new Date(b.finalDueDate + "T23:59:59").getTime() : Infinity;
      return aTime - bTime;
    });
  }

  if (sortOrder === "newest") {
    return list.sort((a, b) => new Date(b.createdAtISO) - new Date(a.createdAtISO));
  }
...
function updateTask(taskId, payload) {
  ...
  if (payload.bufferDueDate && payload.finalDueDate && new Date(payload.bufferDueDate) > new Date(payload.finalDueDate)) {
    alert("予備期限は最終期限より前にしてください");
    return false;
  }
  ...
  task.bufferDueDate = payload.bufferDueDate || "";
  task.finalDueDate = payload.finalDueDate || "";
