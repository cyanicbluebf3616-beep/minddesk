...
function getDualDeadlineRingData(task, now = Date.now()) {
  ...
  if (safeNow <= createdAt) {
    ...
    outerProgress = clamp((safeNow - createdAt) / phase1Total);
    innerProgress = 0;
  } else if (safeNow < finalDue) {
    outerProgress = 1;
    innerProgress = clamp((safeNow - bufferDue) / phase2Total);
  } else {
    outerProgress = 1;
    innerProgress = 1;
  }
  ...
  const outerColor = getRingColorByRatio(safeNow < bufferDue ? outerProgress : 1);
  const innerColor = getRingColorByRatio(safeNow < bufferDue ? 0 : innerProgress);
