...
/* =========================================================
   優先カード用 表示補助
========================================================= */
function formatPriorityDueText(task) {
  if (typeof task.daysLeft === "number") {
    if (task.daysLeft < 0) {
      return `超過 ${Math.abs(task.daysLeft)}日`;
    }
    if (task.daysLeft === 0) {
      return "今日まで";
    }
    return `残り ${task.daysLeft}日`;
  }

  return task.finalDueDate || task.bufferDueDate || "未設定";
}

function formatPriorityIdleText(task) {
  if (typeof task.idleDays === "number" && task.idleDays > 0) {
    return `${task.idleDays}日放置`;
  }

  if (typeof task.idleHours === "number" && task.idleHours >= 1) {
    return `${task.idleHours}時間更新なし`;
  }
...
/* =========================================================
   今やるべき3つ カード描画
========================================================= */
function renderTopPriorityCard(task, index) {
  ...
  const rankClass = index === 0 ? "priority-rank-first" : "";

  let toneClass = "priority-tone-low";
  if (risk >= 100) {
    toneClass = "priority-tone-critical";
  } else if (risk >= 60) {
    toneClass = "priority-tone-high";
  } else if (risk >= 30) {
    toneClass = "priority-tone-mid";
  }

  const actionButtons = currentMode === "manager"
    ? `
      <button class="btn btn-sub" onclick="changeTaskStatus('${task.id}', 'In Progress')">開始</button>
      <button class="btn btn-warn" onclick="changeTaskStatus('${task.id}', 'Blocked')">停止</button>
      <button class="btn btn-primary" onclick="changeTaskStatus('${task.id}', 'Done')">完了</button>
      <button class="btn btn-sub" onclick="openTaskDetail('${task.id}')">詳細</button>
    `
    : `
      <button class="btn btn-sub" onclick="quickSetStatus('${task.id}','In Progress')">開始</button>
      <button class="btn btn-warn" onclick="sendBlockedReport('${task.id}')">詰まり</button>
      <button class="btn btn-sub" onclick="sendProgressReport('${task.id}')">進捗</button>
      <button class="btn btn-primary" onclick="openTaskDetail('${task.id}')">詳細</button>
    `;

  return `
    <div class="priority-item ${toneClass} ${index === 0 ? "priority-item-first" : ""}">
      <div class="priority-rank ${rankClass}">#${index + 1}</div>

      <div class="priority-main">
        <div class="priority-title-row">
          <strong>${escapeHtml(task.title)}</strong>
          <span class="priority-risk-badge">
            <span class="priority-risk-label">危険度</span>
            <span class="priority-risk-value">${risk}</span>
          </span>
        </div>
        ...
        <div class="priority-meta">
          ...
          <span>状態: ${escapeHtml(getStatusDisplayLabel(task.status))}</span>
          ${pendingCount ? `<span>未確認報告: ${pendingCount}件</span>` : ""}
        </div>

        <div class="priority-metrics">
          <div class="priority-metric">
            <span class="priority-metric-label">期限</span>
            <span class="priority-metric-value">${escapeHtml(formatPriorityDueText(task))}</span>
          </div>
          <div class="priority-metric">
            <span class="priority-metric-label">放置</span>
            <span class="priority-metric-value">${escapeHtml(formatPriorityIdleText(task))}</span>
          </div>
          <div class="priority-metric">
            <span class="priority-metric-label">報告</span>
            <span class="priority-metric-value">${pendingCount ? `${pendingCount}件待ち` : "なし"}</span>
          </div>
          <div class="priority-metric">
            <span class="priority-metric-label">優先</span>
            <span class="priority-metric-value">${escapeHtml(task.priorityLabel || task.priority || "通常")}</span>
          </div>
        </div>
        ...
        <div class="chips">
          ${
            reasons.length
              ? reasons.slice(0, 3).map((reason) => `<span class="chip">${escapeHtml(reason)}</span>`).join("")
...
function renderDashboard(elements) {
  ...
  if (elements.topPriorityList) {
    ...
    if (topTasks.length === 0) {
      ...
      topTasks.slice(0, 3).forEach((task, index) => {
        elements.topPriorityList.insertAdjacentHTML(
          "beforeend",
          renderTopPriorityCard(task, index)
        );
      });
    }
  }
